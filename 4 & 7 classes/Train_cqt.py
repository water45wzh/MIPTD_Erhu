...
def train(**kwargs):
    ...
    print("Loading data..")
    # =================================================================
    data = cqt(opt.train_data_pth)
    lengths = [int(len(data)*0.75), int(len(data)*0.25)]

    train_mel_dataset, test_mel_dataset = torch.utils.data.dataset.random_split(data, lengths)
    train_dataloader = Dataloader(train_mel_dataset,batch_size = opt.batch_size,shuffle =True,num_workers = opt.num_workers)
